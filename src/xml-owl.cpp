...
OwlReader::OwlReader(VymModel* m)
    : BaseReader(m)
{
    //qDebug() << "Constr. OwlReader";
...
bool OwlReader::read(QIODevice *device)
{
    xml.setDevice(device);

    if (xml.readNextStartElement()) {
        if (xml.name() == QLatin1String("RDF")) {
            readOwlMap();
        } else {
            xml.raiseError("No RDF element was found.");
        }
    }
    return !xml.error();
}

void OwlReader::readOwlMap()
{
    Q_ASSERT(xml.isStartElement() && xml.name() == QLatin1String("RDF"));
...
//    }
    ...
    if (loadMode == File::NewMap || loadMode == File::DefaultMap) {
        ...
        //readVymMapAttr();
        // "author", "title", "comment", "branchCount";
        // Maybe count the number of owl:Class elements for branchCount?
//        if (branchesTotal > 10) {
//            useProgress = true;
...
//        }
        ...
        if (loadMode == File::ImportReplace) {
            if (!lastBranch) {
                xml.raiseError("readOwlMap - Import/Replace map, but nothing selected!");
                return;
            }
            ...
            if (!pb) {
                xml.raiseError("readOwlMap - No parent branch for selection in ImportReplace!");
                return;
            }
    ...
    while (xml.readNextStartElement()) {
        if (xml.name() == QLatin1String("Ontology"))
            readOwlOntology();
        else if (xml.name() == QLatin1String("Class")) {
            readClass(loadMode, insertPos);
            insertPos++;
        }
        else {
            qDebug() << "OwlReader::readOwlMap Unknown Element: " << xml.name();
...
void OwlReader::readOwlOntology()
{
    Q_ASSERT(xml.isStartElement() && xml.name() == QLatin1String("Ontology"));

    QString s = attributeToString("rdf:about");
    if (!s.isEmpty())
        qDebug() << "OwlReader::readOwlOntology about: " << s;

    // children to read: prefix, label, comment
    while (xml.readNextStartElement()) {
        if (xml.name() == QLatin1String("label")) {
            QString s = xml.readElementText();
            qDebug() << "OwlReader::readOwlOntology label: " << s;
            // TODO
        } else if (xml.name() == QLatin1String("comment")) {
            QString s = xml.readElementText();
            qDebug() << "OwlReader::readOwlOntology comment: " << s;
            // TODO
        } else if (xml.name() == QLatin1String("prefix")) {
            QString s = attributeToString("rdf:datatype");
            qDebug() << "OwlReader::readOwlOntology prefix: " << s;
            xml.skipCurrentElement();
            // TODO
        }
        else if (xml.name() == QLatin1String("imports")) {
            QString s = attributeToString("rdf:resource");
            qDebug() << "OwlReader::readOwlOntology imports: " << s;
            xml.skipCurrentElement();
            // Skip for now
        }
        else {
            qDebug() << "OwlReader::readOwlOntology Unknown element: " << xml.name();
            //raiseUnknownElementError();
            // Ignore unknonwn elements
        }

        //xml.skipCurrentElement(); // Why?
    }

}


void OwlReader::readMapDesign()
{
...
void OwlReader::readMapDesignElement()  // FIXME-2 parse the elements, either in VymModel or directly (generic) in MapDesign
{
...
void OwlReader::readMapDesignCompatibleAttributes()
{
...
void OwlReader::readAttribute() // FIXME-3 Checking types no longer needed. Check with firefox export/import
{
...
void OwlReader::readClass(File::LoadMode loadModeBranch, int insertPosBranch)
{
    Q_ASSERT(xml.isStartElement() &&
            (xml.name() == QLatin1String("Class")));
    ...
    if( !s.isEmpty() )
    {
        // store this class (lastBranch) in a map with s as the key so we can connect sub-classes as branches
        if( branchesByResource.count(s) == 0 )
            branchesByResource[s] = lastBranch;
        else
            qDebug() << "OwlReader::readClass duplicate resource: " << s;
    }

    qDebug() << "OwlReader::readClass about: " << s;

    // While going deeper, no longer "import" but just load as usual
    while (xml.readNextStartElement()) {
        if (xml.name() == QLatin1String("label")) {
            QString s = xml.readElementText();
            qDebug() << "OwlReader::readClass label: " << s;
            lastBranch->setHeading(s);
        }
        else if (xml.name() == QLatin1String("comment")) {
            QString s = xml.readElementText();
            qDebug() << "OwlReader::readClass comment: " << s;
            lastBranch->setNote(s);
        } else if (xml.name() == QLatin1String("subClassOf")) {
            QString s = attributeToString("rdf:resource");
            qDebug() << "OwlReader::readClass parent: " << s;
            xml.skipCurrentElement();
            // Find existing branch or mapCenter that matches s. Make this one a branch of that one.
            auto parent_iter = branchesByResource.find(s);
            if( parent_iter != branchesByResource.end() )
            {
                BranchItem *parent = parent_iter->second;
                //parent->appendChild(lastBranch);
                bool succ = model->relinkBranch(lastBranch, parent);
                if( !succ )
                {
                    qDebug() << "Failed to link branches from " << lastBranch->headingPlain() << " to " << parent->headingPlain();
                }

            }
        }
        else {
            qDebug() << "OwlReader::readClass Unknown Element: " << xml.name();
            //raiseUnknownElementError();
            //return;
            xml.skipCurrentElement();
        }
...
void OwlReader::readHeadingOrVymNote()
{
    Q_ASSERT(xml.isStartElement() &&
...
void OwlReader::readLegacyXLink()
...
void OwlReader::readStandardFlag()
{
    Q_ASSERT(xml.isStartElement() &&
...
void OwlReader::readUserFlagDef()
{
    Q_ASSERT(xml.isStartElement() &&
...
void OwlReader::readOrnamentsAttr()
{
    Q_ASSERT(xml.isStartElement() && (
            xml.name() == QLatin1String("branch") ||
            xml.name() == QLatin1String("mapcenter") ||
